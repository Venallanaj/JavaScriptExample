let animal = 'dog',
    isHungry = 'true',
    isYoung = 'true',
    age = 12;

console.log(animal);
if (isHungry) {
    console.log("It's " + isHungry + "Yes he's hungry")
}

if (isYoung) {
    console.log("if is " + isYoung + " years old he is very young")
}


let firstName = "Vena";
lastName = prompt("What is his last name?");
console.log(firstName + " " + lastName);
alert(firstName + " " + lastName);


//va
var testVar = 2; // Declaration & Assignment
testVar = 3; // Assignment (mutation)
testVarB = 3; // Declaration & Assignment
var testVar = 3; // Re-Declaration & Assignment




//let

let testVarC = 4;
//let testVarC = 5;
testVarC = 5;

// Cannot redecale a let


//const
const testVarD = 5;
//testVarD = 8; 
//Cannot reassign or redeclare a const


let testVareE; // undefinde
testVareE = null; // null
console.log(testVareE);